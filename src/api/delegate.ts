...
/**
 * 发起代理：当前用户（代理人）向 grantedUserId（被代理人）发起。
 * 已存在 PENDING/ACCEPTED 幂等；REVOKE 会重置为 PENDING。
 */
export function createDelegate(grantedUserId: number): Promise<null> {
  return apiPost<null>('/api/v1/user/delegate/create', { operateId: grantedUserId })
}

/**
 * 接受代理：当前用户作为被代理人，接受 delegatorUserId（发起方/代理人）的 PENDING 关系。
 * 注意 operateId 是发起方 userId，不是自己。
 */
export function acceptDelegate(delegatorUserId: number): Promise<null> {
  return apiPost<null>('/api/v1/user/delegate/accept', { operateId: delegatorUserId })
}

/**
 * 撤销代理（PENDING/ACCEPTED → REVOKE）。
 * relation 解释 counterpartUserId 是谁：
 * - MANAGING：我是代理人撤销，operateId=被代理人 ID
 * - MANAGED：我是被代理人撤销/拒绝，operateId=代理人 ID
 */
